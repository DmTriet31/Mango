...
// Khởi tạo client với intents cơ bản + MessageContent
...
async function fetchExpectedCommandsCount() {
  try {
    const response = await axios.get('https://server-backend-tdpa.onrender.com/api/expected-commands-count', { timeout: 5000 });
    return response.data.expectedCommandsCount;
  } catch (error) {
    console.log("[WARNING] Unable to fetch expected commands count:", error.message);
    return -1;
  }
}
...
// =======================
// Fetch & Register Commands
// =======================
const fetchAndRegisterCommands = async () => {
  try {
    const response = await axios.get('https://server-backend-tdpa.onrender.com/api/commands', { timeout: 5000 });
    const commands = response.data;

    commands.forEach(command => {
      command.source = 'DmTriet';
      client.commands.set(command.name, {
        ...
        execute: async (interaction) => {
          try {
            ...
            console.log(`[ERROR] Failed executing command ${command.name}:`, error.message);
          }
        }
      });
    });

    console.log("[INFO] Commands fetched and registered successfully.");
  } catch (error) {
    console.log("[WARNING] Failed to fetch commands, continuing without remote commands.");
  }
};
